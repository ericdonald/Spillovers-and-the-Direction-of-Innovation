...
def τ_root(x_lag, x_t, x_lead, T, ρ, var_θ, φ_hat, γ, χ, η, r, α, σ, λ, ν, L, ω, C_bar, var_ρ, ψ_p, ψ_0, ψ, sum_Em_out, Θ, SCC_frac, o):
    ...
    τ_1_t = np.exp(x_t[:,0].reshape((T,1)))
    τ_2_t = np.exp(x_t[:,1].reshape((T,1)))
    C_1_t = np.exp(x_t[:,2].reshape((T,1)))
    C_2_t = np.exp(x_t[:,3].reshape((T,1)))
    A_t = np.exp(x_t[:,4+J:4+2*J])
    
    τ_1_lead = np.exp(x_lead[:,0].reshape((T,1)))
    τ_2_lead = np.exp(x_lead[:,1].reshape((T,1)))
    A_lead = np.exp(x_lead[:,4+J:4+2*J])
...
def C_root(x_lag, x_t, x_lead, T, ρ, var_θ, φ_hat, γ, χ, η, r, α, σ, λ, ν, L, ω, C_bar, var_ρ, ψ_p, ψ_0, ψ, sum_Em_out, Θ, SCC_frac, o):
    ...
    C_1_lag = np.exp(x_lag[:,2].reshape((T,1)))
    C_2_lag = np.exp(x_lag[:,3].reshape((T,1)))
...
def ξtilde_root(x_lag, x_t, x_lead, T, ρ, var_θ, φ_hat, γ, χ, η, r, α, σ, λ, ν, L, ω, C_bar, var_ρ, ψ_p, ψ_0, ψ, sum_Em_out, Θ, SCC_frac, o):
    ...
    ξtilde_lead = x_lead[:,4:4+J]
    A_lead = np.exp(x_lead[:,4+J:4+2*J])
    ς_1_lead = x_lead[:,-2].reshape((T,1))
    ς_2_lead = x_lead[:,-1].reshape((T,1))
...
def ς_root(x_lag, x_t, x_lead, T, ρ, var_θ, φ_hat, γ, χ, η, r, α, σ, λ, ν, L, ω, C_bar, var_ρ, ψ_p, ψ_0, ψ, sum_Em_out, Θ, SCC_frac, o):
    ...
    C_2_t = np.exp(x_t[:,3].reshape((T,1)))
    A_t = np.exp(x_t[:,4+J:4+2*J])
    ς_1_t = x_t[:,-2].reshape((T,1))
    ς_2_t = x_t[:,-1].reshape((T,1))
