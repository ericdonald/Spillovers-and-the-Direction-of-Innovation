...
class Processor:
    ...
    def SpillAnalysis(self, year_start=1975, year_end=2015, tbin=5):
        ...
        m1 = gpf.run_reg(spill_panel["phi_tilde"], spill_panel[["phi_lag"]], 'sm', cluster_groups)
    
        m2 = gpf.run_reg(spill_panel["phi_tilde"], spill_panel[["phi_lag", "clean_trend"]], 'sm', cluster_groups)
        ...
        Percentile = gpf.clean_round((Cent < Cent[0]).mean()*100, 1)
    ...
    def SpillReg(self, year_start=1972, reg_start=1980, year_end=2023, δ_A=0.15, tback=4, tfor=5):
        ...
        def stock_block(s):
            x = s.to_numpy(float)
            out = x.copy()
            out[0] = (1+self.E.g) * x[0] / (δ_A + self.E.g)
        ...
        # OLS #
        # --- #
        tech_panel_df = tech_panel_df[(tech_panel_df["year"] >= reg_start)]
